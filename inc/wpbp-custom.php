...
function airtime_init()
{
	load_theme_textdomain('airtime', THEME_DIRECTORY . '/lang');

	add_image_size('airtime_square', 400,  400, true);
	add_image_size('airtime_mini',   200,  100, true);
	add_image_size('airtime_small',  400,  200, true);
	add_image_size('airtime_medium', 800,  400, true);
	add_image_size('airtime_large',  1200, 600, true);
	add_image_size('airtime_cover',  2000, 400, true);

	add_editor_style('editor-style.css');
}
add_action('init', 'airtime_init');


function airtime_sidebars_init()
{
	wpbp_register_sidebars(array( "Footer" ));
}
add_action('widgets_init', 'airtime_sidebars_init');


function airtime_compile_lesscss()
{
...
if ( isset($_GET['recompile_css']) || is_user_logged_in() ) {
	add_action('init', 'airtime_compile_lesscss');
}


function airtime_default_options()
{
	return array(
...
function airtime_add_style_select_buttons($buttons)
{
	array_unshift($buttons, 'styleselect');
	return $buttons;
}
add_filter('mce_buttons_2', 'airtime_add_style_select_buttons');


function airtime_custom_styles($init_array)
{
	$style_formats = array(
		array(
			'title'   => __("Heading Number", 'airtime'),
			'inline'  => 'span',
...
add_filter('tiny_mce_before_init', 'airtime_custom_styles');


function airtime_post_main_category_name($post_id = false)
{
